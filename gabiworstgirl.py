        ...
        if comment.author == 'not_yet_eh':
            ...
            if ru4 in comment.body:
       	    ...
       	    if ru5 in comment.body:
                x = x.replace("$randomuser", random.choice(users))
        if "randomcharacter" or "randomleak" or "randomship" or "randomtitan" or "randomalive" or "randomlivingboy" or "randomlivinggirl" or "randomboy" or "randomgirl" not in comment.body and comment.author != 'not_yet_eh':
            if ru1 in comment.body:
                x = (comment.body).replace("!randomuser", random.choice(users))
            if ru2 in comment.body:
                x = x.replace("?randomuser", random.choice(users))
