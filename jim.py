...
# reddit api login
reddit = praw.Reddit(client_id='vmfyoV7qWVg01w',
                     client_secret='D1X-llUa-hhFg3gg1551cFQiLAk',
                     username='SandEnthusiast',
                     password='notenoughspace629',
                     user_agent='thisshitdoesntmatterlel')


# the subreddits you want your bot to live on
subreddit = reddit.subreddit('aaa719ljz')
...
for comment in subreddit.stream.comments():
    if not comment.saved:
        x = "bot machine broke"
        if rc1 in comment.body:
            x = (comment.body).replace("!randomcharacter", random.choice(characters))
        if rc2 in comment.body:
            x = x.replace("?randomcharacter", random.choice(characters))
        if rc3 in comment.body:
            x = x.replace("+randomcharacter", random.choice(characters))
        if rc4 in comment.body:
            x = x.replace("-randomcharacter", random.choice(characters))
        if rc5 in comment.body:
            x = x.replace("$randomcharacter", random.choice(characters))
